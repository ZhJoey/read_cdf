...
E_MAG_TL   = 1.079
E_REPT_TL2 = 2.1     # NEW: your text says 2.1 MeV for REPT panel 4/7; keep 2.1 here
E_REPT_TL3 = 4.2
E_REPT_TP  = 2.1     # used by new L–Pitch panels

# Use pitch bin nearest to this angle for Time–L:
PITCH_SELECT_DEG = 70.0

# Color limits
#   Panels 1–3 (log10 flux per keV)
LOGR_MAG_TL   = (0, 4.0)
LOGR_REPT_TL2 = (-1, 3)
LOGR_REPT_TL3 = (-2, 1)
#   Panels 4–6 (log10 normalized flux wrt PA≈90°)
LOGR_LP_NORM  = (-2.0, 2.0)   # NEW: adjust as needed

# Render non-positive flux (≤0)?
SHOW_NONPOS_AS_FLOOR = True
EPS_FLOOR            = 1e-6   # tiny floor used for floor/normalization, per keV
...
OUT_NAME = f"rbsp_7panel_mageis_rept_{START_DATE}_{END_DATE}_perkeV_normPA.png"  # NEW
...
BZ_CODE    = 16   # WORD 17: Bz_GSM (nT)  (not used now)
PDYN_CODE  = 28   # WORD 29: Flow pressure P (nPa) (not used now)
NP_CODE    = 24   # WORD 25: Proton density (cm^-3) (not used now)
V_CODE     = 25   # WORD 26: Bulk speed (km/s) (not used now)
# ==========================================================================
...
def load_day(path: Path):
    ...
    Returns dict:
      ...
      flux_tpe_perkeV (N x P x E)        # NEW: per keV
    """
    ...
    # ---- per keV (NOT per MeV)  ---------------------------------------- NEW
    units = atts.get("UNITS", "cm^-2 s^-1 sr^-1 keV^-1")
    ulow = units.lower()
    # Heuristic: rely on declared units first; fall back to energy scale hint
    if ("kev^-1" in ulow) or ("per kev" in ulow) or (" kev^-1" in ulow) or (scale.lower() == "keV"):
        F_perkeV = F
    elif ("mev^-1" in ulow) or ("per mev" in ulow) or (" mev^-1" in ulow) or (scale.lower() == "meV"):
        F_perkeV = F / 1000.0  # per MeV → per keV
    else:
        # If ambiguous, decide by magnitude + energy scale; prefer not to surprise
        F_perkeV = F if "kev" in ulow else (F / 1000.0 if "mev" in ulow else F)

    return dict(time=t, L=L, pitch=A, energy_MeV=np.asarray(energy_MeV, float),
                flux_tpe_perkeV=np.asarray(F_perkeV, float))   # NEW
...
# -------- NEW: linear (not log) time×pitch at a requested energy ------------
def time_pitch_linear_at_energy(F_tpe, E_vec, Ereq_mev):
    ...
    return M  # linear units (per keV)

# -------- NEW: build L–Pitch normalized scatter points ----------------------
def l_pitch_norm_points(F_tpe, E_vec, pitch_vec, L_vec, Ereq_mev,
                        ref_pitch_deg=90.0, exclude_ref_point=False):
    """
    Build L–Pitch scatter for a given energy using strict normalization:
        R(alpha) = F(alpha) / F(90°)   (same time row)
    - Works in *linear* space first, then log10 the ratio.
    - Rows with non-positive or non-finite F(90°) are skipped.
    - Optionally drop the reference (90°) points from the scatter.

    Returns flattened arrays: L (x), pitch (y), log10(R) (c).
    """
    # 1) Extract time×pitch flux at requested energy (linear, per keV)
    M = time_pitch_linear_at_energy(F_tpe, E_vec, Ereq_mev)  # shape: (time, pitch)

    # 2) Find the 90° bin; require it to be "close enough"
    pidx_ref = nearest_pitch_idx(pitch_vec, ref_pitch_deg)
    pref = float(pitch_vec[pidx_ref])
    if abs(pref - ref_pitch_deg) > 7.5:
        print(f"[warn] Closest pitch to {ref_pitch_deg}° is {pref:.1f}°. "
              f"Check your pitch grid.")

    # 3) Row-wise reference and strict normalization
    ref = M[:, pidx_ref]                                  # (time,)
    good_row = np.isfinite(ref) & (ref > 0)               # only valid denominators
    Norm = np.full_like(M, np.nan, dtype=float)
    with np.errstate(invalid="ignore", divide="ignore"):
        Norm[good_row, :] = M[good_row, :] / ref[good_row, None]
        # Ensure exact 1 at the reference column (purely cosmetic)
        Norm[good_row, pidx_ref] = 1.0

    # 4) Flatten to scatter vectors
    T, P = M.shape
    Lrep = np.repeat(np.asarray(L_vec, float), P)
    Prep = np.tile(np.asarray(pitch_vec, float), T)
    R    = Norm.ravel()

    # mask invalid & (optionally) drop the 90° column itself
    m = np.isfinite(Lrep) & np.isfinite(Prep) & np.isfinite(R) & (R > 0)
    if exclude_ref_point:
        m &= (np.abs(Prep - pref) > 1e-6)

    return Lrep[m], Prep[m], R[m]  # color = log10(F/F_90)
...
if __name__ == "__main__":
    ...
    rept_F *= 0.5  # keep your original scaling
    ...
    # ---- NEW: L–Pitch normalized scatter data (per keV, normalized by PA≈90°)
    mag_Lx, mag_Py, mag_C = l_pitch_norm_points(mag_F, mag_E, mag_A, mag_L, E_MAG_TL, ref_pitch_deg=90.0)
    rept2_Lx, rept2_Py, rept2_C = l_pitch_norm_points(rept_F, rept_E, rept_A, rept_L, E_REPT_TL2, ref_pitch_deg=90.0)
    rept4_Lx, rept4_Py, rept4_C = l_pitch_norm_points(rept_F, rept_E, rept_A, rept_L, E_REPT_TL3, ref_pitch_deg=90.0)
    ...
    fig, axes = plt.subplots(
        7, 1, figsize=(12, 18), sharex=False,            # NEW: 7 panels
        gridspec_kw={"height_ratios":[1,1,1,1,1,1,0.9]}  # NEW: tune as you like
    )
    fig.subplots_adjust(left=LEFT_MARGIN, right=RIGHT_MARGIN,
                        top=TOP_MARGIN, bottom=BOTTOM_MARGIN, hspace=HSPACE)
    fig.suptitle(
        f"{PROBE.upper()} Electron Flux (MagEIS/REPT)  — per keV; PA-norm @ 90°",
        fontsize=FS_SUP
    )

    extend_kw = 'min' if SHOW_NONPOS_AS_FLOOR else 'neither'
    CBAR_LABEL_FLUX = r"log$_{10}$(cm$^{-2}$ s$^{-1}$ sr$^{-1}$ keV$^{-1}$)"  # NEW
    CBAR_LABEL_NORM = r"log$_{10}$(Flux / Flux$_{90^\circ}$)"                 # NEW

    # -------- Panels 1–3: Time–L (per keV) ---------------------------------
    # Panel 1: MagEIS 1.079 MeV
    ax = axes[0]
    ...
    if np.any(ok):
        ...
        ax.set_ylim(lo, hi)
    ax.set_xlim(0.0, float((t_mag.max().ceil("D") - base_day_utc) / pd.Timedelta(days=1)))
    ...
    if np.any(ok):
        ...
        ax.set_ylim(lo, hi)
    ax.set_xlim(0.0, float((t_rept.max().ceil("D") - base_day_utc) / pd.Timedelta(days=1)))
    ...
    # -------- Panels 4–6: L–PitchAngle (normalized by PA≈90°) --------------
    # Panel 4: MagEIS 1.079 MeV
    ax = axes[3]
    sc4 = ax.scatter(mag_Lx, mag_Py, c=mag_C, s=6, cmap="jet",
                     vmin=LOGR_LP_NORM[0], vmax=LOGR_LP_NORM[1])
    ax.set_ylabel("Pitch (deg)")
    ax.set_xlim(0.8, 10.0)
    ax.set_ylim(0, 180)
    ax.text(ENERGY_LABEL_X, 0.5, f"{E_MAG_TL:g} MeV", transform=ax.transAxes,
            rotation=90, va="center", ha="right", fontsize=FS_LABEL)
    cb4 = fig.colorbar(sc4, ax=ax, pad=0.01, fraction=0.04, extend=extend_kw); cb4.ax.tick_params(labelsize=FS_TICK)

    # Panel 5: REPT 2.1 MeV
    ax = axes[4]
    sc5 = ax.scatter(rept2_Lx, rept2_Py, c=rept2_C, s=6, cmap="jet",
                     vmin=LOGR_LP_NORM[0], vmax=LOGR_LP_NORM[1])
    ax.set_ylabel("Pitch (deg)")
    ax.set_xlim(0.8, 10.0)
    ax.set_ylim(0, 180)
    ax.text(ENERGY_LABEL_X, 0.5, f"{E_REPT_TL2:g} MeV", transform=ax.transAxes,
            rotation=90, va="center", ha="right", fontsize=FS_LABEL)
    cb5 = fig.colorbar(sc5, ax=ax, pad=0.01, fraction=0.04, extend=extend_kw); cb5.ax.tick_params(labelsize=FS_TICK)

    # Panel 6: REPT 4.2 MeV
    ax = axes[5]
    sc6 = ax.scatter(rept4_Lx, rept4_Py, c=rept4_C, s=6, cmap="jet",
                     vmin=LOGR_LP_NORM[0], vmax=LOGR_LP_NORM[1])
    ax.set_ylabel("Pitch (deg)")
    ax.set_xlim(0.8, 10.0)
    ax.set_ylim(0, 180)
    ax.text(ENERGY_LABEL_X, 0.5, f"{E_REPT_TL3:g} MeV", transform=ax.transAxes,
            rotation=90, va="center", ha="right", fontsize=FS_LABEL)
    cb6 = fig.colorbar(sc6, ax=ax, pad=0.01, fraction=0.04, extend=extend_kw); cb6.ax.tick_params(labelsize=FS_TICK)

    # Shared cb label for the three above:
    fig.text(0.93, 0.37, CBAR_LABEL_NORM, rotation=90, va="center", ha="center", fontsize=FS_CBAR)

    # -------- Panel 7: Kp & Dst (x_hours) -----------------------------------
    ax7 = axes[6]
    ax7.grid(True, alpha=0.3, linestyle=":")
    ax7.set_ylabel("Dst (nT)")
    # Reuse tick_pos/labels from earlier day grid for readability
    ax7.set_xticks(tick_pos)
    ax7.set_xticklabels(tick_lab)
    ax7.xaxis.set_minor_locator(mticker.MultipleLocator((TICK_STEP_HOURS/24.0)/2.0))
    ...
    # Tidy x-lims for time–L panels to shared day window
    for ax in axes[:3]:
