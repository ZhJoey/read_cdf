...
# -*- coding: utf-8 -*-
"""
RBSP/REPT time–L plots at pitch≈90° (per keV) over a date range + Kp/Dst

- Finds daily L3 CDFs in a directory tree (year subfolders)
- Loads FEDU/FPDU + Epoch + L/L_star + Energy + Pitch grid
- Selects the pitch-angle bin closest to 90° (no averaging)
- Converts flux from per MeV → per keV
- Uses *specific* energies you set; if not an exact channel, does log-linear
  interpolation in energy (set EXACT_ONLY=True to forbid)
- Plots 3 panels (Time × L colored by log10 flux per keV) + 4th panel with
  Dst (left y) and Kp (right y) from OMNIWeb
"""
...
# Your *specific* energies (in MeV). If not exact channel, interpolate (see EXACT_ONLY)
ENERGIES_MEV = [1.8, 3.4, 4.2]
EXACT_ONLY   = False                # set True to raise if any E isn't an exact channel

# Plot style
LOG_VMIN, LOG_VMAX = 1.0, 4.0       # colorbar range for log10(flux per keV)
CMAP = "jet"

SAVE_PLOT = True
SAVE_NAME = "rept_timeL_pitch90_keV_kpdst_testMev.png"
# ==========================================================================
...
def to_dt_list_any(cdf_epoch_array):
    """Robust: return list of naive Python datetimes from CDF epochs."""
...
def clean_fill(arr, fillval):
    """Replace FILLVAL with NaN (and absurd magnitudes) in a numeric array copy."""
...
def pick_flux_var(cdf, prefer=None):
    """Choose a flux-like variable (prefer FEDU then FPDU)."""
...
def find_file_for_date(day: datetime) -> Path | None:
    """Find daily file for a date (YYYY/YYYYMMDD) and choose latest version if requested."""
...
def _fetch_omni_series(start_dt, end_dt, varnum):
    """
    Download hourly OMNI2 variable via nx1.cgi (staging .lst). Returns tz-aware UTC Series.
    varnum: 38=Kp*10, 40=Dst
    """
...
# ------------------------------- REPT loader ------------------------------
def load_rept_day(path: Path, prefer_flux=None, pitch_target_deg=90.0):
    """
    Load one daily CDF. Returns dict with:
      time (list datetime), L (cleaned), energy_vec_MeV, flux_te_per_keV (time x energy)
    """
    ...
    if pitch_dep:
        try:
            ...
            pitch_vec = None
    if pitch_vec is None:
        raise RuntimeError(f"{path.name}: pitch axis present but no pitch vector loaded.")

    # flux array
    flux = clean_fill(cdf.varget(flux_name), fv_flux)

    # reorder to (time, pitch, energy)
    if flux.ndim == 3:
        # move time to axis 0 by matching len(time)
        ...
        # decide pitch vs energy axes by length match
        if flux.shape[2] == len(energy_vec_MeV):
            ...
            pitch_axis, energy_axis = 1, 2  # fallback
        # select pitch bin nearest to target
        pv = np.array(pitch_vec, dtype=float).ravel()
        # handle possible 0–360 wrap (most REPT files are 0–180 though)
    ...
    # Convert per MeV → per keV
    flux_te_per_keV = np.array(flux_te, dtype=float) / 1000.0

    return dict(
        time=np.array(time, dtype=object),
        L=L,
        energy_vec_MeV=energy_vec_MeV,
        flux_te_per_keV=flux_te_per_keV,
        flux_name=flux_name,
    )

# -------------------------- energy selection ------------------------------
def concat_flux_per_keV(parts):
    """Stack (time,energy) flux from all days along time axis."""
    return np.concatenate([p["flux_te_per_keV"] for p in parts], axis=0)

def series_at_specific_energies(flux_te_per_keV, energy_vec_MeV, energies_req_MeV,
                                tol=1e-6, exact_only=False):
    """
    For each requested E, return a time series at *that energy*:
      - if exact channel center (within tol): take that column
      - else: log-linear interpolation between bracketing channels (unless exact_only)
    Returns: list of 1D arrays, and meta list [(Ereq, 'exact' or 'interp e0–e1 MeV'), ...]
    """
    ...
    for Ereq in energies_req_MeV:
        ...
        if exact_idx.size > 0:
            col = flux_te_per_keV[:, exact_idx[0]].astype(float)
        ...
        # must be bracketed for interpolation
        ...
        f0 = flux_te_per_keV[:, lo].astype(float)
        f1 = flux_te_per_keV[:, hi].astype(float)

        out = np.full_like(f0, np.nan, dtype=float)
        m = (f0 > 0) & (f1 > 0)
        out[m] = np.exp(np.log(f0[m]) * (1.0 - w) + np.log(f1[m]) * w)

        # if only one side valid, use it
...
if __name__ == "__main__":
    ...
    # Load days (pitch≈90°, per keV)
    parts = []
    for p in paths:
        try:
            parts.append(load_rept_day(p, FLUX_VAR, pitch_target_deg=PITCH_SELECT_DEG))
        except Exception as e:
            print(f"[SKIP] {p.name}: {e}")
    if not parts:
        raise SystemExit("[ABORT] No usable REPT files.")

    # Concatenate time/L and flux
    time_all = np.concatenate([p["time"] for p in parts])
    L_all    = np.concatenate([p["L"]   for p in parts])
    flux_all_per_keV = concat_flux_per_keV(parts)
    ...
    flux_series_list, energy_meta = series_at_specific_energies(
        flux_all_per_keV, energy_vec_MeV, ENERGIES_MEV,
        tol=1e-6, exact_only=EXACT_ONLY
    )
    ...
    # Build tz-aware time for plotting and Kp/Dst downloads
    ...
    fig.suptitle(f"{PROBE.upper()}  Time–L flux (pitch≈{PITCH_SELECT_DEG:.0f}°) per keV + Kp/Dst\n"
                 f"{START_DATE} to {END_DATE} (UTC)")

    # Panels 1–3: time–L scatter colored by log10 flux per keV
    for ax, logf, meta in zip(axes[:3], log_flux_list, energy_meta):
        Ereq, how = meta
        E_keV = int(round(Ereq * 1000))
        # mask: valid flux & valid L
        ok = np.isfinite(logf) & np.isfinite(L_all)
        sc = ax.scatter(x_time[ok], L_all[ok], c=logf[ok], s=4,
                        cmap=CMAP, vmin=LOG_VMIN, vmax=LOG_VMAX)
        # choose reasonable L limits from data
        if np.any(ok):
