...
v6:
- Per-panel colorbar ranges via CUSTOM_LOG_RANGES (log10 scale) in ENERGIES order
- Adds top padding on L-axis so max doesn't touch the frame
- Tight x-axis: no space before first or after last timestamp

Other features kept:
- ENERGY_UNIT = "MeV" or "keV"; ENERGIES interpreted in that unit
- Flux per selected unit (per MeV / per keV)
- Exact-or-interpolated energies along energy axis
- Clean L fill masking
- Kp/Dst from OMNI (hourly)
"""
...
# Exact channel only? (False = log-linear interpolation if no exact match)
EXACT_ONLY  = False

# ---- NEW: per-panel colorbar ranges (log10 of flux per selected unit) ----
# Provide one (vmin, vmax) per energy, in the same order as ENERGIES.
# Example (your request): 1.8→2–6, 3.8→2–6, 4.2→1–4
CUSTOM_LOG_RANGES = [(2, 7), (1, 5), (1, 5)]
# If you set this to None, the global defaults below are used for all panels.
LOG_VMIN_DEFAULT, LOG_VMAX_DEFAULT = 1.0, 4.0

# Y-axis top padding (extra headroom so the top points aren't flush)
Y_TOP_PAD_ABS  = 0.20   # add this many L-shell units
...
# Save output?
SAVE_PLOT = True
SAVE_NAME = f"rept_timeL_pitch90_{ENERGY_UNIT}_4.png"
# ==========================================================================
...
if __name__ == "__main__":
    print(f"[CONFIG] ENERGY_UNIT={ENERGY_UNIT}, ENERGIES={ENERGIES}")
    energies_req_MeV = energies_to_MeV(ENERGIES, ENERGY_UNIT)
    print(f"[INFO] Interpreted energies (MeV): {energies_req_MeV}")
    ...
    for i, (ax, logf, (Ereq_MeV, how), E_disp) in enumerate(
        ...
        # Per-panel colorbar on the right (slim)
        cbar = fig.colorbar(sc, ax=ax, pad=0.01, fraction=0.05)
        cbar.set_label(rf"log$_{{10}}$ (cm$^{{-2}}$ s$^{{-1}}$ sr$^{{-1}}$ {unit_label}$^{{-1}}$)")
    ...
    # Tight x-limits & zero margins on bottom panel too
    ...
    # A little room for per-panel colorbars
    fig.subplots_adjust(right=0.92, hspace=0.32)
