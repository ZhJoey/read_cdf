...
v5:
- Robust ENERGY_UNIT handling ("MeV" or "keV")
- ENERGIES are interpreted in that unit and converted to MeV internally
- Debug prints show interpreted energies (MeV)
- Guards against common mistakes (e.g., keV list used with MeV unit)
"""
...
# ---- Unit + energies ----
ENERGY_UNIT = "keV"                # "MeV" or "keV" (case-insensitive)
ENERGIES    = [1800, 3400, 4200]   # interpreted in ENERGY_UNIT
...
def energies_to_MeV(energies, unit: str):
    """
    Convert a list of energies (given in 'unit') to MeV (float list).
    Also guard against common mistakes and print what we end up using.
    """
    ...
    # Heuristics to help when user mislabels unit/list:
    if unit_l == "mev":
        # If numbers look like keV (>= 100), warn and auto-convert
        ...
        # If numbers look like MeV (<= 50), warn and auto-convert
        if np.nanmax(e) <= 50.0:
            print("[warn] ENERGIES look like MeV values but ENERGY_UNIT='keV'. Auto-converting MeV→keV first, then to MeV.")
            # they intended MeV list; convert MeV→keV for display, but we need MeV internally
            # just treat them as MeV and return directly:
            return e.tolist()
        # Proper keV list → convert to MeV
        return (e / 1000.0).tolist()
...
if __name__ == "__main__":
    # Normalize and convert energies to MeV internally
    ...
    # values for titles: in selected unit
    if unit_label == "keV":
    ...
    for ax, logf, (Ereq_MeV, how), E_disp in zip(
        axes[:3], log_flux_list, energy_meta, energy_display_vals
    ):
