...
# Color limits (log10 flux per MeV)
LOGR_MAG_TL   = (-4.0, 1.0)
LOGR_REPT_TL2 = (2.0, 6.0)
LOGR_REPT_TL3 = (1.0, 4.0)
LOGR_REPT_TP  = (2.0, 6.0)
LOGR_REPT_TP_NORM = (-1.0, 1.0)  # log10(Flux/Flux90): 0.01–10×
...
OUT_NAME = f"rbsp_6panel_mageis_rept_{START_DATE}_{END_DATE}_PADnorm2-NoLog.png"
...
if __name__ == "__main__":
    ...
    rept_log_tp = time_pitch_at_energy(rept_F, rept_E, E_REPT_TP)

    # --- REPT Time–Pitch @ E_REPT_TP, normalized to ~90° ---
    rept_tp_linear = time_pitch_at_energy_linear(rept_F, rept_E, E_REPT_TP)  # linear flux
    p90_idx = nearest_pitch_idx(rept_A, PITCH_SELECT_DEG)
    ref = rept_tp_linear[:, p90_idx]  # (time,)
    # Avoid division by nonpositive/NaN refs
    # valid_ref = np.isfinite(ref) & (ref > 0)
    # norm = np.full_like(rept_tp_linear, np.nan, dtype=float)
    # norm[valid_ref, :] = rept_tp_linear[valid_ref, :] / ref[valid_ref, None]
    norm = rept_tp_linear / np.maximum(ref[:, None], EPS_FLOOR)

    # Log (base 10) for plotting, using existing floor behavior
    rept_log_tp_norm = to_log10_with_floor(norm, show_floor=SHOW_NONPOS_AS_FLOOR, eps=EPS_FLOOR)
    rept_tp_norm = norm   # keep linear Flux/Flux90
    ...
    # ---------- Bz & Pdyn (numeric codes; compute Pdyn if needed) ----------
    pdyn_bz = None
    try:
    ...
    fig, axes = plt.subplots(
        6, 1, figsize=(12, 15), sharex=True,
        gridspec_kw={"height_ratios":[1,1,1,1.2,0.7,0.7]}
    )
    fig.subplots_adjust(left=LEFT_MARGIN, right=RIGHT_MARGIN,
                        top=TOP_MARGIN, bottom=BOTTOM_MARGIN, hspace=HSPACE)
    # fig.suptitle(
    #     f"{PROBE.upper()}  MagEIS+REPT: Time–L (0.909/2.1/4.2 MeV), Time–Pitch (2.1 MeV), Kp/Dst, Pdyn/Bz\n"
    ...
    CBAR_LABEL = r"log$_{10}$(cm$^{-2}$ s$^{-1}$ sr$^{-1}$ MeV$^{-1}$)"

    # Panel 1: MagEIS 0.909 MeV
    ax = axes[0]
    ok = np.isfinite(mag_log_tl1) & np.isfinite(mag_L)
    sc1 = ax.scatter(t_mag[ok], mag_L[ok], c=mag_log_tl1[ok], s=6, cmap="jet",
    ...
    ax.text(ENERGY_LABEL_X, 0.5, f"{E_MAG_TL:g} MeV (MagEIS)", transform=ax.transAxes,
    ...
    sc2 = ax.scatter(t_rept[ok], rept_L[ok], c=rept_log_tl2[ok], s=6, cmap="jet",
                     vmin=LOGR_REPT_TL2[0], vmax=LOGR_REPT_TL2[1])
    ax.set_ylabel("L"); ax.set_xlim(xmin, xmax); ax.margins(x=0)
    if np.any(ok):
        lo, hi = np.nanpercentile(rept_L[ok], [1,99]); lo=max(0.8,lo); hi=min(10.0,hi+0.15*(hi-lo))
        ax.set_ylim(lo, hi)
    ax.text(ENERGY_LABEL_X, 0.5, f"{E_REPT_TL2:g} MeV (REPT)", transform=ax.transAxes,
            rotation=90, va="center", ha="right", fontsize=FS_LABEL)
    ...
    sc3 = ax.scatter(t_rept[ok], rept_L[ok], c=rept_log_tl3[ok], s=6, cmap="jet",
                     vmin=LOGR_REPT_TL3[0], vmax=LOGR_REPT_TL3[1])
    ax.set_ylabel("L"); ax.set_xlim(xmin, xmax); ax.margins(x=0)
    if np.any(ok):
        lo, hi = np.nanpercentile(rept_L[ok], [1,99]); lo=max(0.8,lo); hi=min(10.0,hi+0.15*(hi-lo))
        ax.set_ylim(lo, hi)
    ax.text(ENERGY_LABEL_X, 0.5, f"{E_REPT_TL3:g} MeV (REPT)", transform=ax.transAxes,
            rotation=90, va="center", ha="right", fontsize=FS_LABEL)
    ...
    fig.text(0.93, 0.64, CBAR_LABEL, rotation=90, va="center", ha="center", fontsize=FS_CBAR)

    # Panel 4: REPT Time–Pitch @ 2.1 MeV
    # ax = axes[3]
    # pm = ax.pcolormesh(t_rept, rept_A, rept_log_tp.T, shading="nearest", cmap="jet",
    #                    vmin=LOGR_REPT_TP[0], vmax=LOGR_REPT_TP[1])
    # ax.set_ylabel("Pitch (deg)")
    # ax.set_xlim(xmin, xmax); ax.margins(x=0)
    # cb4 = fig.colorbar(pm, ax=ax, pad=0.01, fraction=0.04, extend=extend_kw); cb4.ax.tick_params(labelsize=FS_TICK)
    # ax.text(ENERGY_LABEL_X, 0.5, f"{E_REPT_TP:g} MeV (REPT)", transform=ax.transAxes,
    #          rotation=90, va="center", ha="right", fontsize=FS_LABEL)
    
    # Panel 4: REPT Time–Pitch @ 2.1 MeV (normalized to ~90°)
    # ax = axes[3]
    # pm = ax.pcolormesh(t_rept, rept_A, rept_log_tp_norm.T, shading="nearest", cmap="jet",
    #                 vmin=LOGR_REPT_TP_NORM[0], vmax=LOGR_REPT_TP_NORM[1])
    # ax.set_ylabel("Pitch (deg)")
    # ax.set_xlim(xmin, xmax); ax.margins(x=0)
    # cb4 = fig.colorbar(pm, ax=ax, pad=0.01, fraction=0.04, extend=extend_kw); cb4.ax.tick_params(labelsize=FS_TICK)
    # ax.text(ENERGY_LABEL_X, 0.5, f"{E_REPT_TP:g} MeV (REPT)\n(norm. to ~{PITCH_SELECT_DEG:.0f}°)", 
    #         transform=ax.transAxes, rotation=90, va="center", ha="right", fontsize=FS_LABEL)

    # Panel 4: REPT Time–Pitch @ 2.1 MeV (normalized to ~90°, linear ratio)
    ax = axes[3]
    pm = ax.pcolormesh(t_rept, rept_A, rept_tp_norm.T, shading="nearest", cmap="jet",
                    vmin=0.0, vmax=2.0)   # adjust range as needed
    ax.set_ylabel("Pitch (deg)")
    ax.set_xlim(xmin, xmax); ax.margins(x=0)
    cb4 = fig.colorbar(pm, ax=ax, pad=0.01, fraction=0.04)
    cb4.ax.tick_params(labelsize=FS_TICK)
    cb4.set_label(r"Flux / Flux$_{90^\circ}$", fontsize=FS_CBAR)
    ...
    if kp_dst is not None:
        ...
        df = df.reindex(hours).interpolate(limit_direction="both")
        ax5.plot(df.index, df["Dst"], color="k", lw=1.5, label="Dst")
        ax5r = ax5.twinx()
        ax5r.step(df.index, df["Kp"], where="post", color="tab:orange", lw=1.5, label="Kp")
    ...
    if pdyn_bz is not None:
        ...
        df.index = pd.to_datetime(df.index, utc=True)
        df = df.reindex(hours).interpolate(limit_direction="both")
        ax6.plot(df.index, df["Pdyn"], color="tab:blue", lw=1.5, label="Pdyn")
        ax6r = ax6.twinx()
        ax6r.plot(df.index, df["Bz"], color="tab:red", lw=1.2, label="Bz")
